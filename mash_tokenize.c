...
/* no definition of this in Windows yet */
...
static size_t strlcpy(char *dest, const char *src, size_t bufferSize) {
    size_t nCopied;
...
static char *operatorChar(int c) {
    if (c == '|')
        return "|";
    if (c == '<')
        return "<";
 ...
 */
static char *getTokenFromBuffer(char **buffer, int *pushedChar) {
     ...
     */
    for (i = 0; tokenString[i] != '\0' && (!isblank(tokenString[i])) && (tokenString[i] != '\n') &&
...
char *tokensToString(char *buf, int bufsiz, char **const tokens, int useQuotes) {
    ...
    *buf = 0;
     ...
     */
    if (strlcat(buf, tokens[0], bufsiz) >= bufsiz)
        goto OVERFLOW;
    for (i = 1; tokens[i] != NULL; i++) {
        ...
        if (useQuotes)
            if (strlcat(buf, "'", bufsiz) >= bufsiz)
                goto OVERFLOW;
        if (strlcat(buf, tokens[i], bufsiz) >= bufsiz)
            goto OVERFLOW;
        if (useQuotes)
            if (strlcat(buf, "'", bufsiz) >= bufsiz)
                goto OVERFLOW;
